...
# 邮件安全等级（推荐 self-strict）
EMAIL_SECURITY_LEVEL=self-strict
# 可选：收件人白名单（逗号分隔）
EMAIL_ALLOWED_RECIPIENTS=your@qq.com
...
> 若目标是“只发给自己”，请将 `EMAIL_TO` 与 `EMAIL_FROM` 配置为同一邮箱，并保持 `EMAIL_SECURITY_LEVEL=self-strict`。
...
- 默认安全等级为 `self-strict`，会拦截“非本人收件人/多收件人/非SSL”配置
