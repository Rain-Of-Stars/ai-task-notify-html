...
def _validate_email_policy(
    ...
    - open: 仅做白名单校验（如配置）
    - self: 必须单收件人且与发件人一致
    - self-strict: 在 self 基础上强制 SSL 和安全端口
    """
    level = get_config(env, "EMAIL_SECURITY_LEVEL", "self-strict").strip().lower()
    if not level:
        level = "self-strict"

    sender = _normalize_email(email_from)
    normalized_recipients = [_normalize_email(r) for r in recipients]
    normalized_recipients = [r for r in normalized_recipients if r]

    if not sender:
        return False, "EMAIL_FROM 格式无效"
    if not normalized_recipients:
        return False, "EMAIL_TO 格式无效"

    allowed_raw = _split_csv(get_config(env, "EMAIL_ALLOWED_RECIPIENTS", ""))
    allowed = {_normalize_email(a) for a in allowed_raw if _normalize_email(a)}
    if allowed and any(r not in allowed for r in normalized_recipients):
        return False, "收件人不在白名单 EMAIL_ALLOWED_RECIPIENTS 中"

    if level in ("self", "self-strict"):
        if len(normalized_recipients) != 1:
            return False, "当前安全等级仅允许单收件人"
        if normalized_recipients[0] != sender:
            return False, "当前安全等级要求收件人与发件人一致"

    if level == "self-strict":
        if not use_ssl:
            return False, "self-strict 要求 SMTP_USE_SSL=true"
        if smtp_port not in (465, 587):
            return False, "self-strict 仅允许 SMTP 端口 465 或 587"

    if level not in ("open", "self", "self-strict"):
        return False, "EMAIL_SECURITY_LEVEL 仅支持 open/self/self-strict"
...
def extract_conversation(data: dict, source: str) -> list[dict]:
    ...
    def _join_texts(texts: list[str]) -> str:
        """合并消息分片，避免把结构化文本撑出过大的空白间距"""
        return "\n".join(t.rstrip() for t in texts if t is not None and t != "")
    ...
    if source == "claude-code":
        for item in data.get("transcript", []):
            ...
            if texts:
                messages.append({"role": role, "text": _join_texts(texts)})
        ...
        for item in input_msgs:
            if isinstance(item, str) and item.strip():
                ...
                if isinstance(content, str) and content.strip():
                    ...
                    if texts:
                        messages.append({"role": "user", "text": _join_texts(texts)})
            ...
            if isinstance(content, str) and content.strip():
                ...
                for c in content:
                    if isinstance(c, dict) and c.get("type") == "text":
                        ...
                        texts.append(c.strip())
                if texts:
                    messages.append({"role": "assistant", "text": _join_texts(texts)})
        ...
        if stripped.startswith("```"):
            if in_code:
                ...
                html_parts.append(
                    ...
                    'word-wrap:break-word;max-width:100%;overflow-x:auto;overflow-y:hidden;">'
                    + code_text
                    + "</pre>"
                )
                code_lines = []
...
def build_email_html(title: str, source: str, data: dict) -> str:
    ...
    for key, value in meta_rows:
        meta_html += (
            ...
            f"width:32%;white-space:normal;vertical-align:top;"
    ...
    for msg in conversation:
        ...
        conversation_html += (
            '<tr><td class="px24" style="padding:0 24px 12px;">'
            '<table width="100%" cellpadding="0" cellspacing="0"'
            ' border="0" role="presentation" style="border-collapse:collapse;">'
            ...
            f'overflow-wrap:anywhere;word-break:break-word;word-wrap:break-word;">'
            f"{content_html}"
            f"</div>"
            f"</td></tr></table>"
            f"</td></tr>"
    ...
    if not conversation_html:
        ...
        conversation_html = (
            '<tr><td class="px24" style="padding:0 24px 16px;">'
            ...
            'word-wrap:break-word;max-width:100%;overflow-x:auto;">'
...
<head>
  ...
  <style type="text/css">
    ...
    @media screen and (max-width:640px){{
      ...
      .outer-pad{{padding:24px 4%!important;}}
    ...
    @media screen and (max-width:480px){{
      ...
      .title{{font-size:18px!important;line-height:24px!important;overflow-wrap:anywhere!important;word-break:break-word!important;}}
      .badge{{display:inline-block!important;margin-top:8px!important;}}
    }}
  </style>
</head>
<body style="margin:0;padding:0;width:100%!important;background-color:#F3F4F6;
  ...
  <table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation"
    ...
    <tr><td align="center" class="outer-pad" style="padding:32px 4%;">

      <!-- 主卡片 -->
      <table width="100%" class="container" cellpadding="0" cellspacing="0" border="0" role="presentation"
        ...
        <!-- 标题区 -->
        <tr><td class="px24" style="padding:24px 24px 16px;">
          <table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation"
                 style="border-collapse:collapse;width:100%;">
            <tr>
              <td class="stack" style="vertical-align:middle;">
                <span class="title" style="font-size:20px;font-weight:700;color:#111827;
                             overflow-wrap:anywhere;word-break:break-word;">
                  {_escape_html(title)}</span>
              </td>
              <td class="stack" align="right" style="vertical-align:middle;">
                <span class="badge" style="display:inline-block;padding:4px 12px;
        ...
        <!-- 分割线 -->
        <tr><td class="px24" style="padding:0 24px;border-top:1px solid #E5E7EB;
                       font-size:0;line-height:0;">&nbsp;</td></tr>

        <!-- 元数据 -->
        <tr><td class="px24" style="padding:16px 24px;">
          <table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation"
                 style="border-collapse:collapse;background:#F9FAFB;
                        border-radius:8px;border:1px solid #E5E7EB;table-layout:fixed;">
            {meta_html}
          </table>
        </td></tr>

        <!-- 对话区域标题 -->
        <tr><td class="px24" style="padding:8px 24px 12px;">
        ...
        <!-- 底部 -->
        <tr><td class="px24" style="padding:16px 24px;background:#F9FAFB;
...
def send_email(
    ...
    allowed, reason = _validate_email_policy(
        env=env,
    ...
    if not allowed:
        print(f"Email policy blocked: {reason}", file=sys.stderr)
        return False
    ...
    try:
        ...
        if use_ssl:
            server = smtplib.SMTP_SSL(
                smtp_host, smtp_port, timeout=10, context=ssl_ctx
            )
        else:
